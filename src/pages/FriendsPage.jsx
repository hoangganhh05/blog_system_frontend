...
function getMutualFriendsMap() {
  try {
    return JSON.parse(localStorage.getItem("blog_mutual_friends_pairs") || "[]");
  } catch {
    return [];
  }
}
...
function addMutualFriendPair(userId1, userId2) {
  try {
    ...
    const exists = pairs.some(
      (p) => (p.u1 === userId1 && p.u2 === userId2) || (p.u1 === userId2 && p.u2 === userId1)
    );
    if (!exists) {
...
function getPendingRequests() {
  try {
    return JSON.parse(localStorage.getItem("blog_pending_friend_requests") || "[]");
  } catch {
    return [];
  }
}
...
function sendFriendRequestStore(fromId, toId) {
  try {
    ...
    if (!exists) {
      ...
      localStorage.setItem("blog_pending_friend_requests", JSON.stringify(reqs));
    }
  } catch {}
}
...
function removePendingRequest(fromId, toId) {
  try {
    const reqs = getPendingRequests();
    const updated = reqs.filter((r) => !(r.fromId === fromId && r.toId === toId));
    localStorage.setItem("blog_pending_friend_requests", JSON.stringify(updated));
  } catch {}
}
              ...
              {(activeTab === "all" || activeTab === "requests") && (
                <div style={{ marginBottom: 32 }}>
                  ...
                  {incomingRequests.length === 0 ? (
                    <div style={{ fontSize: 14, color: "var(--text-muted)", background: "var(--bg-card)", padding: 16, borderRadius: 12 }}>
              ...
              {(activeTab === "all" || activeTab === "friends") && (
                <div>
                  <h3 style={{ fontSize: 18, fontWeight: 800, margin: "0 0 16px 0", color: "var(--text-primary)" }}>
                    Tất cả bạn bè 2 chiều ({friendsList.length})
                  </h3>

                  {friendsList.length === 0 ? (
                    <div style={{ fontSize: 14, color: "var(--text-muted)", background: "var(--bg-card)", padding: 16, borderRadius: 12 }}>
                      Bạn chưa kết bạn 2 chiều với ai. Hãy bấm "Thêm bạn bè" từ danh sách gợi ý để kết bạn!
                    </div>
                  ) : (
