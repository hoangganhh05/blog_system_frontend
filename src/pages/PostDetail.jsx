...
function PostDetail() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (currentUser?.id) {
          ...
          const checkB = await bookmarkService.checkBookmarked(id, currentUser.id);
  ...
  const handleSelectReaction = async (type) => {
    ...
    try {
      ...
      const msg = err.response?.data?.message || err.response?.data || err.message || "Lỗi tương tác!";
    ...
    <div className="app-layout">
      <div className="post-detail-container">
        ...
        <div style={{ marginBottom: 16, display: "flex", alignItems: "center", gap: 8, color: "var(--text-muted)", fontSize: 13.5 }}>
          ...
          {post.category && (
            <>
              <span style={{ color: "var(--primary)", fontWeight: 500 }}>{post.category.name}</span>
              <span>›</span>
            </>
          )}
          ...
          <div className="post-detail-body" style={{ padding: 24 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
              ...
              >
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12.5, color: "var(--text-muted)", display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", marginTop: 2 }}>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 16 }}>
              <button
                ...
                style={{ borderRadius: 20, fontWeight: 600, background: "var(--primary-light)", color: "var(--primary)", border: "1px solid var(--primary)" }}
              >
                ⚡ {summaryLoading ? "AI đang tóm tắt..." : summaryPoints.length > 0 ? "Ẩn tóm tắt AI" : "AI Tóm Tắt Bài Viết"}
              </button>

              <button
                onClick={handleToggleSpeech}
                className="btn btn-secondary btn-sm"
                style={{ borderRadius: 20, fontWeight: 600, border: isSpeaking ? "1.5px solid #ef4444" : "1px solid var(--border)", color: isSpeaking ? "#ef4444" : "var(--text-primary)" }}
              >
            ...
            {summaryPoints.length > 0 && (
              ...
              >
                <div style={{ fontWeight: 700, color: "var(--primary)", fontSize: 14, marginBottom: 8, display: "flex", alignItems: "center", gap: 6 }}>
                  <span>⚡ Tóm tắt thông minh bởi AI Assistant:</span>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 6, fontSize: 13.5, color: "var(--text-primary)" }}>
            ...
            {post.sharedPost ? (
              ...
              <div style={{ marginTop: 10 }}>
                {post.content && (
                  <p style={{ fontSize: 16, color: "var(--text-primary)", marginBottom: 16, lineHeight: 1.5 }}>
                    {post.content}
                  </p>
                )}
                  ...
                  {post.sharedPost.bgColor ? (
                    ...
                    >
                      <p style={{
                        color: "#fff",
                        textAlign: "center",
                        fontSize: post.sharedPost.content?.length < 80 ? 20 : 16,
                        fontWeight: 700,
                        lineHeight: 1.5,
                        margin: 0,
                    ...
                    <>
                      ...
                      <p style={{ fontSize: 14.5, color: "var(--text-secondary)", lineHeight: 1.45 }}>
                        {post.sharedPost.content}
                      </p>
                    </>
                  )}
              ...
              <>
                <h1 className="post-detail-title" style={{ fontSize: 24, fontWeight: 800, marginBottom: 12 }}>
                  {post.title}
                </h1>
                <div className="post-detail-content" style={{ fontSize: 15.5, lineHeight: 1.6, color: "var(--text-primary)" }}>
                  {post.content}
                </div>
              </>
            )}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                {likeCount > 0 && (
                  <>
                    ...
                    <span style={{ fontWeight: 600, color: "var(--text-secondary)" }}>{likeCount}</span>
                  </>
                )}
              </div>
            ...
            >
              ...
              {showReactionsPicker && (
                <div
                  ...
                  style={{
                    ...
                    gap: 6,
                    zIndex: 200,
                    animation: "reactionPopIn 0.22s cubic-bezier(0.175, 0.885, 0.32, 1.275)",
                  }}
                >
                  {REACTIONS.map((r) => (
                    <button
                      ...
                      onMouseEnter={(e) => (e.currentTarget.style.transform = "scale(1.35) translateY(-6px)")}
              ...
              <div style={{ flex: 1, display: "flex" }}>
                <button className="post-action-btn" onClick={scrollToComments} style={{ width: "100%", gap: 6 }}>
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
